...
assert.ok(css.includes("--loss-ok:rgb(96 176 132)"), "packet-loss tender green missing")
...
assert.ok(source.includes("function trafficBarColor(percent)"), "quota traffic color ramp helper missing")
assert.ok(source.includes("if(p<=30)return 'var(--traffic-ok)'"), "quota traffic must stay green through 30 percent")
assert.ok(css.includes("--traffic-ok:rgb(96 176 132)"), "traffic bar must start with the same tender green")
