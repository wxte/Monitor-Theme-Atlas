...
assert.ok(source.includes('metaBits=[node.arch,node.virt].filter(Boolean)'), "card meta must only show arch and virtualization text")
assert.ok(source.includes('cpuValue=m?'), "card CPU header must include core count with utilization")
